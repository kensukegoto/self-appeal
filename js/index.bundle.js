...
var Resizer = function Resizer(func) {
  var _this = this;
  ...
  window.addEventListener("resize", function () {
    ...
    _this.timer = setTimeout(function () {
      if (_this.w === window.innerWidth) return;
      if (_this.w > 959 && window.innerWidth > 959) return;
...
/* harmony import */ var _wordcloud__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./wordcloud */ "../script/wordcloud.js");
 ...
 */
(function () {
  var gnavi = document.querySelector(".m-gnavi");
  var btn = document.querySelector(".c-humburger");
  ...
  */
  ...
  var items = gnavi.querySelectorAll(".item");
  ...
  try {
    for (var _iterator = items[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      item.addEventListener("click", function (e) {
        ...
        if (target !== "#top") {
          // 画面上部から要素までの距離
          var rectTop = document.querySelector(target).getBoundingClientRect().top; // 現在のスクロール距離

          var offsetTop = window.pageYOffset; // スクロール位置に持たせるバッファ

          var buffer = window.innerWidth > 959 ? 80 : 60;
...
Object(_wordcloud__WEBPACK_IMPORTED_MODULE_7__["default"])({
  selector: ".c-cloud",
  csv: "./data/word.csv"
});
...
/* harmony import */ var _Resizer___WEBPACK_IMPORTED_MODULE_24__ = __webpack_require__(/*! ./Resizer  */ "../script/Resizer .js");
...
function _ref() {
  _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(c) {
    var _c, selector, csv, w, h, m, shuffle, data, color, fontSizeSet, fontSize, svg, wc, cloud, resize, draw;

    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            draw = function _ref6(words) {
              wc.selectAll("g").remove();
              var words2 = words.reduce(function (acc, e, i) {
                if (acc[i % 3]) {
                  acc[i % 3].push(e);
                } else {
                  acc[i % 3] = [];
              ...
              words2.forEach(function (list, i) {
                var g = wc.append("g").attr("class", "g".concat(i + 1));
                g.selectAll("text").data(list).enter().append("text").style("fill", function (d, i) {
                  return color(i);
                ...
                TweenMax.from(".g".concat(i + 1), [3, 1, 2][i], {
                  y: w > 959 ? [-10, 5, 10][i] : [-20, 10, 15][i],
                  // CSSプロパティ
                  repeat: -1,
                  yoyo: true,
                  ease: "none" // イージング
            ...
            resize = function _ref5() {
              ...
              fontSize = d3.scalePow().range(fontSizeSet[w > 959 ? 1 : 0]);
              svg.attr("width", w).attr("height", h);
              wc.attr("transform", "translate(".concat(w / 2, ",").concat(h / 2 + m.t, ")"));
              cloud.size([w, h - m.t - m.b]).words(data).rotate(0).fontSize(function () {
                return fontSize(Math.random());
              }).fontWeight(["bold"]).font(["Cabin"]).text(function (d) {
                return d.Name;
              }).on("end", draw).start();
            };

            _c = _objectSpread({}, c), selector = _c.selector, csv = _c.csv;
            w = document.querySelector(selector).clientWidth;
            h = document.querySelector(selector).clientHeight;
            m = w > 959 ? {
              t: 20,
            ...
            shuffle = function shuffle(_ref2) {
          ...
          case 9:
            data = _context.sent;
            data = shuffle(data).filter(function (e, i) {
              return i < 25;
            });
            color = d3.scaleOrdinal(d3.schemePaired);
            fontSizeSet = [[20, 40], [60, 120]];
            fontSize = d3.scalePow().exponent(5).domain([0, 1]) // 0 ~ 1の入力値
            .range(fontSizeSet[w > 959 ? 1 : 0]); // 5乗なのでより40に近い値が取得される傾向になる

            svg = d3.select(selector).append("svg").attr("width", w).attr("height", h);
            wc = svg.append("g").attr("class", "wordcloud").attr("transform", "translate(".concat(w / 2, ",").concat(h / 2 + m.t, ")"));
            cloud = d3.layout.cloud().size([w, h - m.t - m.b]);
            cloud.words(data).rotate(0).fontSize(function () {
              return fontSize(Math.random());
            }).fontWeight(["bold"]).font(["Cabin"]).text(function (d) {
              return d.Name;
            }).on("end", draw).start();
            new _Resizer___WEBPACK_IMPORTED_MODULE_24__["default"](resize); // draw
